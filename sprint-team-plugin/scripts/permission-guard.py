...
def main():
    ...
    if state_file.exists():
        try:
            with open(state_file, encoding="utf-8") as f:
                current_phase = json.load(f).get("current_phase", "")
        except (json.JSONDecodeError, OSError):
            pass
    ...
    if allowed:
        write_guard_event(sprint_id, "permission-guard", file_path, "-", role, current_phase)
        sys.exit(0)
    else:
        write_guard_event(sprint_id, "permission-guard", file_path, reason, role, current_phase)
